...
class ProductionHybrid3DEngine:
    ...
    async def _ai_enhance_result(self, rendered_image: np.ndarray, original_image: np.ndarray,
        ...
        """
        STAGE 4: AI Enhancement using Stable Diffusion
        """
        try:
            if self.ai_enhancer is None:
                logger.warning("AI enhancer not available, using traditional enhancement")
                return self._traditional_enhancement(rendered_image, original_image)
            
            logger.info("Applying AI enhancement with Stable Diffusion...")
            
            # Prepare images for AI enhancement
            rendered_pil = Image.fromarray(rendered_image)
            original_pil = Image.fromarray(original_image)
            
            # Create enhancement prompt
            prompt = f"photorealistic person wearing {product_name}, natural lighting, high quality, realistic fabric texture"
            
            # Apply AI enhancement
            enhanced = self.ai_enhancer(
                prompt=prompt,
                image=rendered_pil,
                strength=0.3,  # Light enhancement to preserve original
                guidance_scale=7.5,
                num_inference_steps=20
            ).images[0]
            ...
            final_result = self._preserve_face_features(enhanced_array, original_image)
    ...
    def _traditional_enhancement(self, rendered_image: np.ndarray, original_image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            enhanced = cv2.addWeighted(rendered_image, 0.7, enhanced, 0.3, 0)
            ...
            logger.error(f"Traditional enhancement error: {e}")
            return rendered_image
    
    def _preserve_face_features(self, enhanced_image: np.ndarray, original_image: np.ndarray) -> np.ndarray:
        """Preserve facial features from original image"""
        try:
            # Face detection
            ...
            original_faces = face_cascade.detectMultiScale(gray_original, 1.1, 4)
            enhanced_faces = face_cascade.detectMultiScale(gray_enhanced, 1.1, 4)
            
            result = enhanced_image.copy()
            
            # Blend face regions if detected
            if len(original_faces) > 0 and len(enhanced_faces) > 0:
                # Use largest faces
                orig_face = max(original_faces, key=lambda x: x[2] * x[3])
                enh_face = max(enhanced_faces, key=lambda x: x[2] * x[3])
                
                ox, oy, ow, oh = orig_face
                ex, ey, ew, eh = enh_face
                
                # Extract and resize face regions
                orig_face_region = original_image[oy:oy+oh, ox:ox+ow]
                orig_face_resized = cv2.resize(orig_face_region, (ew, eh))
                
                # Blend face region (preserve original face)
                alpha = 0.8
                result[ey:ey+eh, ex:ex+ew] = (
                    alpha * orig_face_resized + (1 - alpha) * result[ey:ey+eh, ex:ex+ew]
                ).astype(np.uint8)
